...
public class RingAntSystem {
	...
	public RingAntSystem(int colNum){
		colonies = new baseAntSystem[colNum];
		initColonies(colNum);
	}
	...
	private void initColonies(int colNum){
		...
		for(int i=0;i<colonies.length;i++){
			...
			colonies[i] = new baseAntSystem(dm,new AntSystemArgs(ag.size,ag.antNum/colNum+add));
			add=0;
		}
	}
	...
	public static void main(String arg[]){
		TSP t = new TSP(arg[0]);
		dm = t.getMatrix();
		l = new Log("RingLog" + arg[2]);
		ag = new AntSystemArgs(Integer.valueOf(arg[1]),Integer.valueOf(arg[1]));
		l.logger.setLevel(Level.SEVERE);
		

		try {
			System.setOut(new PrintStream(new FileOutputStream("ch130RingAS-bf.csv")));
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		
		RingAntSystem ras = new RingAntSystem(ag.coloniesNumber);
